...
namespace BAI_2_6_XuLyString
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            #endregion Lý thuyết Xử lý chuỗi

            do
            {
                Console.Write("Mời bạn nhập tên: ");
                string name = Console.ReadLine();
                Console.WriteLine(VietHoaFullName(name));
                Console.WriteLine(ZenMaFPOLY(name));
            } while (true);
        }

        //Nguyễn Anh Dũng => dungna
        //nguyễn anh DŨNG => Nguyễn Anh Dũng

        #region Ví dụ xử lý chuỗi

        public static string ZenMaFPOLY(string fullName)//nguyễn Anh DŨNG
        {
            string temp = fullName.Trim().ToLower();// nguyễn anh dũng
            string[] arrName = temp.Split(' ');//Đưa chuỗi về mảng
            string final = VietHoaChuCaiDau(arrName[arrName.Length - 1]).Trim();//Dũng
            for (int i = 0; i < arrName.Length - 1; i++)
            {
                final += GetChuCaiDau(arrName[i]);
            }

            return final;
        }

        public static string VietHoaFullName(string fullName)//nguyễn Anh DŨNG
        {
            string temp = fullName.Trim().ToLower();// nguyễn anh dũng
            string[] arrName = temp.Split(' ');//Đưa chuỗi về mảng
            string final = "";
            ...
            {
                final += VietHoaChuCaiDau(arrName[i]);
            }

            return final.Trim();//Nguyễn Anh Dũng
        }

        public static string GetChuCaiDau(string value)//Nguyễn
        {
            string temp = value.Trim().ToLower();//nguyễn
            return temp[0].ToString();//n
        }

        public static string VietHoaChuCaiDau(string value)//nguyen,NGUỸEN
        {
            string temp = value.Trim().ToLower();//nguyễn
            return temp[0].ToString().ToUpper() + temp.Substring(1) + " ";//Nguyễn
        }

        #endregion Ví dụ xử lý chuỗi
    }
}